                ...
                if (data.error == false) {
                    ...
                    $.each(data.data, function (i, e) {
                        ...
                        html += '<label for="select-address-' + e.id + '" class="form-check-label"><li class="list-group-item d-flex justify-content-between lh-condensed mt-3">' +
                            ...
                            '<small class="col-12 text-muted">' + e.address + ' , ' + e.city + ' , ' + e.state + ' , ' + e.country + ' - ' + e.pincode + '</small>' +
            ...
            success: function (data) {
                ...
                if ((data.promo_codes).length != 0) {
                     $.each(data.promo_codes, function (i, e) {
                        html += '<div class="card mb-2"><label for="promo-code-' + e.id + '"><li class="list-group-item d-flex align-item-center mt-3 p-3">' +
                            '<div class="promo-code-img"><img src="' + e.image + '" class="img-fluid"/></div>' +
                            '<div class="text-start ml-2 flex-grow-1">' +
...
// Function to get city, state, and country from the pincode
const getPincodeDetails = async (pincode) => {
    try {
        ...
        if (data[0].Status === 'Success') {
            ...
            // Auto-fill the city, state, and country fields
...
// Event listener for when the user enters the pincode
document.getElementById('pincode_name').addEventListener('input', function() {
    const pincode = this.value;
    if (pincode.length === 6) {  // Trigger the API call only when the pincode length is 6
