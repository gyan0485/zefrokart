...
$(document).ready(function () {
    ...
    // for redirect to app download link popup
      // Check if the popup should be shown
      if (sessionStorage.getItem('gotoAppPopupClosed')) {
         $('#gotoAppSection').hide();
      }

      // Handle the close button click
      $('.close-goto-app-section').on('click', function() {
         // Hide the popup
         $('#gotoAppSection').hide();
         // Store the preference in localStorage
         sessionStorage.setItem('gotoAppPopupClosed', 'true');
      });
      
      //   checkout section payment option highlighted check
      $('input[name="payment_method"]').change(function() {
        // Remove 'selected' class from all payment options
        $('.highlighted-radio').removeClass('selected');

        // Add 'selected' class to the parent of the checked input
        $(this).closest('.highlighted-radio').addClass('selected');
    });
    ...
    // Fraud
    $("#checkout_form_fraud").on('submit', function (event) {
    ...
    function place_order() {
        let myForm = document.getElementById('checkout_form_fraud');
    ...
    $('#clear_promo_btn').on('click', function (event) {
        ...
        console.log(parseFloat(promocode_amount));
        var new_final_total = parseFloat(sub_total) + parseFloat(delivery_charge) - parseFloat(wallet_used);
        $('#final_total').text(
    ...
    // Fraud
    document.getElementById("address-modal1").addEventListener("show.bs.modal", () => {
        $.ajax({
            type: 'POST',
            data: {
                [csrfName]: csrfHash,
                ...
                if ((data.promo_codes).length != 0) {
                    $.each(data.promo_codes, function (i, e) {
                        html += '<div class="card mb-2"><label for="promo-code-' + e.id + '"><li class="list-group-item d-flex align-item-center mt-3">' +
                            ...
                            '<div class="copy-promo-code d-flex gap-2 p-2 text-dark" title="Copy promocode" id="redeem_promocode" data-value = ' + e.promo_code + '>' + e.promo_code + '<i class="fa fa-copy text-blue"></i></div>' +
                            '<small class="text-muted">' + e.message + '</small>' +
                            '</div>' +
                            '</li></label></div>';
                    });
                } else {
                    // html += '<div class="no-promo-code-img"><img src="' + base_url + '/assets/front_end/modern/img/no-offer-2.jpeg" /></div>' + '<div class="col-12 text-dark d-flex justify-content-center">Opps...No Offers Avilable</small>';
    ...
    // fraud
    $(".address_modal1").on('click', '.submit', function (event) {
        event.preventDefault();
        // var address="";
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('#checkout_form_fraud > .row').unblock()
                ...
                // console.log(delivery_charge_without_cod);
                result.availability_data.forEach(product => {
                    if (product.delivery_by == 'standard_shipping') {
                ...
                $('input[type=radio][name=payment_method]').change(function () {
                    ...
                    if (selectedPaymentMethod === 'COD') {
                        // delivery_charge = parseFloat(delivery_charge_with_cod);
                        if (delivery_charge_with_cod.indexOf(',') !== -1) {
                        ...
                        // console.log(delivery_charge_without_cod);
                        if (delivery_charge_without_cod.indexOf(',') !== -1) {
...
$(document).on('click', '#wallet_balance', function () {
    ...
    if ($(this).is(':checked')) {
        ...
        console.log(delivery_charge_without_cod);
        console.log("if checked then final total is if" + final_total);
        if (final_total - wallet_balance <= 0) {
            ...
            console.log(delivery_charge_without_cod);
            console.log("if checked then final total is else" + final_total);
        ...
        console.log("if not checked then final total is " + final_total);
...
$('.app-navigation-item').each(function() {
...
function addToCart(productId) {
    fbq('track', 'AddToCart', {
        content_ids: [productId], // Dynamic product ID
        content_type: 'product'
    });
}
function trackPurchase(productIds, value) {
    fbq('track', 'Purchase', {
        content_ids: productIds, // Array of product IDs
        value: value, // Total value of the purchase, e.g., 1234.99
        currency: 'INR', // Hardcoded to INR
        content_type: 'product' // Content type
    });
}

function trackViewContent(productIds) {
...
$(".city_new").select2( {
    // ajax: {
    //     url:base_url+"my-account/get_cities", type:"GET", dataType:"json", delay:250, data:function(e) {
    //         return {
    //             search: e.term
    //         }
    //     }
    ...
    // }
    // ,
     minimumInputLength:1, theme:"bootstrap4", placeholder:"Search for cities", containerCssClass:"city-container", dropdownCssClass:"city-dropdown", dropdownParent:$("#add-address-form")
});
