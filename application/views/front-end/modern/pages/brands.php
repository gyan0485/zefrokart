            ...
            <?php foreach ($brands as $key => $row) { ?>
                <div class="swiper-slide-category text-center brand_image_div mx-2">
                    <a href="<?= base_url('products?brand=' . html_escape($row['brand_slug'])) ?>" class="text-decoration-none">
                        <img class="lazy" src="<?= base_url('assets/front_end/modern/img/product-placeholder.jpg') ?>" data-src="<?= base_url('media/image?path='. $row['brand_img'] .'&width=110&quality=100') ?>" alt="<?= html_escape($row['brand_name']) ?>" />
