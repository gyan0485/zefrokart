...
class Auth extends CI_Controller
{
    ...
    public function index()
    {
     ...
     protected function sendResponse($status = true, $message = '') {
        $response = [];
    ...
    protected function sendSms($number, $otp) {
        // $requestParams = array(
        //     'type' => 'text',
        //     'contacts' => $number,
        //     'key' => '63aaa9c2835bc',
        //     'senderid' => 'ZEFROK',
        //     'peid' => '1201166400223088666',
        //     'templateid' => '1207167223372568559',
        //     'msg' => $otp.' is your OTP for verification code on Zefrokart sms service.'
        // );
        $requestParams = array(
            'type' => 'text',
            'contacts' => $number,
            ...
            'senderid' => 'TRUELY',
            'peid' => '1201166400223088666',
            'templateid' => '1607100000000327638',
            'msg' => $otp.' is your OTP Code for Mobile verification on Zefrokart sms'
            );
        
        
        $apiUrl = "http://softsms.in/app/smsapi/index.php?";
        foreach($requestParams as $key => $val){
            $apiUrl .= $key.'='.urlencode($val).'&';
        }
        
        $apiUrl = rtrim($apiUrl, "&"); //API call
    ...
    {
        $this->form_validation->set_rules('mobile', 'Mobile', 'trim|required|xss_clean|min_length[5]|numeric');
        ...
        if( is_exist(['mobile' => $mobile], 'users') ) {
            $otp = mt_rand(100000,900000);
            $this->sendSms($mobile, $otp);
            update_details(['otp' => $otp], ["mobile" => $mobile], 'users');
            $this->sendResponse(true);
        } else {
            $email = $mobile . "@zefrokart.com";
            $additional_data = [
                'username' => $mobile,
                'email' => $email,
                'mobile' => $mobile,
            ...
            $user_id = $this->ion_auth->register($mobile, '123456789', NULL, $additional_data, ['2']);
            $otp = mt_rand(100000,900000);
            $this->sendSms($mobile, $otp);
            update_details(['active' => 1, 'email' => $email, 'otp' => $otp], ["mobile" => $mobile], 'users');
            $this->sendResponse(true);
        }
    }

    public function login_with_otp_verify()
    {
        $this->form_validation->set_rules('mobile', 'Mobile', 'trim|required|xss_clean|min_length[5]|numeric');
        $this->form_validation->set_rules('otp', 'OTP', 'trim|required|xss_clean|min_length[3]|numeric');
        ...
        $user = get_user_inst(['mobile' => $mobile]);

        if( !empty($user) && $user['otp'] == $otp ) {
            if( $user['active'] != '1' ) {
                $this->sendResponse(false, "Your account is inactive at present. Please try after some time.");
                return;
            }
            
            $ses = $this->ion_auth->login($mobile, '123456789', TRUE, TRUE);
            if( $ses == FALSE ) {
                $this->sendResponse(false);
            } else {
                update_details(['otp' => NULL], ["mobile" => $mobile], 'users');
                $this->sendResponse(true);
            }
        }
