        ...
        <form class="needs-validation" id="checkout_form" method="POST" action="<?= base_url('cart/place-order') ?>">
            <div class="row">
                <div class="col-xl-8 bg-white mt-3">
                    ...
                    <div class="ship-details-wrapper mt-3">
                        ...
                        <?php if ($cart[0]['type'] != 'digital_product') { ?>
                            <div class="align-item-center ship-title-details justify-content-between user-add d-flex">
                                <h5 class="pb-1"><?= !empty($this->lang->line('shipping_address')) ? $this->lang->line('shipping_address') : 'Shipping Address' ?></h5>
                                <!--<a href="#" data-izimodal-open=".address-modal"><i class="fas fa-edit edit-icon"></i></a>-->
                                <a target="_blank" href="<?= base_url('my-account/manage-address') ?>" data-toggle="modal" data-target="#address-add-modal"><?= !empty($this->lang->line('create_a_new_address')) ? $this->lang->line('create_a_new_address') : 'Create a New Address' ?></a>
                            </div>
                            ...
                            <div class="shipped-details mt-2">
                                ...
                                <p class="mt-3">
                                    <a href="javascript:void(0);" data-izimodal-open=".address-modal">Select address for delivery</a>
                                </p>
                            </div>
                    ...
                    <?php $time_slot_config['is_time_slots_enabled']= null; ?>
                   <?php if (isset($time_slot_config['is_time_slots_enabled']) && $time_slot_config['is_time_slots_enabled'] == 1 && $cart[0]['type'] != 'digital_product') {
                    ...
                    <div class="align-item-center ship-title-details justify-content-between mb-2 d-flex pb-1 user-add">
                    ...
                    <div class="ship-details-wrapper mt-4 payment-methods">
                        ...
                        <div class="shipped-details mt-3 pl-0 col-md-6">
...
<div class="modal fade iziModal address-add-modal" id="address-add-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="background:rgb(33 37 41 / 41%)!important;">
    <div class="modal-dialog iziModal-wrap modal-lg">
        <div class="modal-content iziModal-wrap">
            <div class="modal-header">
                <h5 style="font-size:1.2rem;font-weight:bold;">Add Address</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close" data-iziModal-close>
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="<?= base_url('my-account/add-address') ?>" method="POST" id="add-address-form" class="p-4 pt-0">
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12 col-xs-12 form-group">
                        ...
                        <select name="city_id" id="city" class="select-city form-control">
                    ...
                    <div class="col-md-6 col-sm-12 col-xs-12 form-group">
                        ...
                        <label for="Zipcode" class="control-label"><?= !empty($this->lang->line('pincode1')) ? $this->lang->line('pincode1') : 'Area/landmark/Road' ?></label>
                        <input type="text" class="form-control" id="pincode" name="landmark" placeholder="Enter Area/landmark/Road" />
                    ...
                    <div class="col-md-12 col-sm-12 col-xs-12 text-center">
                        ...
                        <button class="btn btn-outline-secondary ml-3" data-dismiss="modal" aria-label="Close">Close</button>
                    </div>
                    <div class="col-md-12 col-sm-12 col-xs-12 text-center">
...
<div class="modal fade edit-modal-lg edit-address-modal" id="address-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 style="font-size:1.2rem;font-weight:bold;" class="" id="exampleModalLongTitle"><?= !empty($this->lang->line('edit_address')) ? $this->lang->line('edit_address') : 'Edit Address' ?></h5>
                ...
                <form action="<?= base_url('my-account/edit-address') ?>" method="POST" id="edit-address-form">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 col-sm-12 col-xs-12 form-group">
                            ...
                            <!--</select>-->
                             <label for="pincode" class="control-label"><?= !empty($this->lang->line('landmark')) ? $this->lang->line('landmark') : 'Landmark' ?></label>
                            <input type="text" class="form-control" id="edit_landmark" name="landmark" placeholder="landmark" />
                        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const editAddressFiller = editAddress["click .edit-address"];
        let editAddressId;

        // when add address or edit address modal is closed;
        // refresh the list from the backend
        // $("#address-add-modal,.edit-address-modal").on("hide.bs.modal", function() {
        //     // refresh the address list from the backend in the address list modal
        //     if( window.checkoutAddressModalOnOpenEventFn ) {
        //         checkoutAddressModalOnOpenEventFn();
        //     }
        //      checkoutAddressModalOnOpenEventFn();
        // });
        ...
        $(document).on("click", ".edit-address", function(e) {
            ...
            if( id && checkoutGlobalAddressList && checkoutGlobalAddressList[id] ) {
                editAddressId = id;
                $(".edit-address-modal").modal("show");
            }
        });

        $(document).on("click", ".delete-address2", function(e) {
            e.preventDefault();
            const id = parseInt($(this).attr("data-id"));
            if( id && checkoutGlobalAddressList && checkoutGlobalAddressList[id] ) {
                if( window.checkoutAddressModalDeleteAddress ) {
                    checkoutAddressModalDeleteAddress(id, function() {
                        if( window.checkoutAddressModalOnOpenEventFn ) {
                            checkoutAddressModalOnOpenEventFn();
                        }
                        if( $("#address_id").val() == id ) {
                            $("#address_id").val("");
                            $(".shipped-details > *").text("");
        ...
        $(".edit-address-modal").on("shown.bs.modal", function() {
            if( editAddressId ) {
                $("#edit-address-result").html("").show();
                const data = checkoutGlobalAddressList[editAddressId];
                console.log("data: ", data);
                // having trouble setting id:
                editAddressFiller(null, null, data);
                $("input[id='address_id']").val(data.id);
                $("input[id='edit_pincode']").val(data.pincode);
            }
        });
    });
</script>
